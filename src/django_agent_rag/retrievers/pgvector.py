...
class PGVectorRetriever:
    ...
    def retrieve(self, query: str, top_k: int = 5, filters: dict | None = None) -> list[RetrievedChunk]:
        ...
        queryset = queryset.annotate(distance=distance_cls("vector", vector)).order_by("distance")[:top_k]
        return [
            RetrievedChunk(
                chunk_id=chunk.id,
                ...
                text=chunk.text,
                score=float(getattr(chunk, "distance", 0.0)) if getattr(chunk, "distance", None) is not None else None,
